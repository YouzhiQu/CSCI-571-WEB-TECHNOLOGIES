...
function createSimilarItems(content) {
    ...
    try{
        ...
        if(content.hasOwnProperty("getSimilarItemsResponse")){
            //var response = content["getSimilarItemsResponse"];
...
function makeSellerMessage(content) {
    ...
    //show_seller.appendChild(show_seller_btn);
    var flag = false;
    ...
    show_seller_btn.onclick  = function() {
        if(show_seller.getAttribute('show') == 'false') {
            ...
            if(content.hasOwnProperty("discription")) {
                ...
                //seller_message.height = seller_message.contentWindow.document.body.offsetHeight;
            }
...
function makeSimilarItems(content) {
    ...
    show_similar_btn.onclick  = function() {
        if(show_similar.getAttribute('show') == 'false') {
            ...
            //console.log(similar_message, 999);
    ...
    for (var i = 0; i < keys.length; i++) {
        switch (keys[i]){
            ...
            case 'ReturnPolicy':
                ...
                //obj["shippingType"] = result[keys[i]][0]["shippingType"][0];
                //obj["shippingServiceCost"] = result[keys[i]][0]["shippingServiceCost"][0]["__value__"];
                break;
            case 'ItemSpecifics':
...
function makeDetailNorRow(name, content) {
    ...
    right_td.appendChild(document.createTextNode(content));;
...
function makeDetailPriceRow(content) {
    ...
    right_td.appendChild(document.createTextNode(price));;
    row.appendChild(left_td);
    row.appendChild(right_td);
    return row;
}
...
function createTable(content) {
    ...
    console.log(result);
    //var table_content = convertToContent(result);
    ...
    function createShippingElement(Info) {
        var td = document.createElement('td');
        //console.log(Info["shippingServiceCost"][0]["__value__"]);
        ...
        // var td = document.createElement('td');
        // try{
        //     var cost = Info["shippingServiceCost"][0]["__value__"];
        ...
        //var type = isset(result[keys[i]][0]["shippingType"][0]) ? result[keys[i]][0]["shippingType"][0]: null;
        //var cost = isset(result[keys[i]][0]["shippingServiceCost"][0]["__value__"]) ? result[keys[i]][0]["shippingServiceCost"][0]["__value__"]: null;
